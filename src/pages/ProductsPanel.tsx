...
export function ProductsPanel({ ticketCategory }: ProductsPanelProps) {
  ...
  const products = allProducts.filter(p => p.active && p.stock_quantity > 0).sort((a, b) => a.name.localeCompare(b.name));
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-black whitespace-nowrap transition-colors uppercase tracking-widest border ${selectedCategory === cat
                ? 'bg-primary-500 border-primary-500 text-white shadow-md'
                : 'bg-[var(--bg-base)] border-[var(--border-lg)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-white'
                }`}
            >
      ...
      <div className="flex-1 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-gray bg-[var(--bg-base)]">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full opacity-50">
            ...
            <p className="text-sm font-medium text-[var(--text-muted)]">Aucun produit trouvé</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3">
            {filteredProducts.map((product) => (
              <button
                ...
                className={`flex flex-col text-left p-3.5 rounded-xl transition-all duration-200 border group shadow-sm active:scale-95 ${isLowStock(product)
              ...
              >
                <div className="flex items-start justify-between w-full mb-2 gap-2">
                  <div className="p-1.5 rounded-md bg-[var(--bg-base)] border border-[var(--border)] shrink-0 hidden sm:block">
                    <Package className="w-4 h-4 text-[var(--text-secondary)]" />
                  </div>
                  <h3 className="font-bold text-sm text-white leading-tight truncate w-full" title={product.name}>
                    {product.name}
                  </h3>
                  {isLowStock(product) ? (
                ...
                {product.brand && <p className="text-xs text-[var(--text-muted)] truncate w-full mb-2">{product.brand}</p>}

                <div className="mt-auto pt-2 w-full border-t border-[var(--border)] flex justify-between items-end">
                  <div>
                    <span className="text-sm font-black text-primary-500">{product.unit_price} DA</span>
                  </div>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${isLowStock(product) ? 'bg-danger-500/20 text-danger-400 border-danger-500/30' : 'bg-success-500/20 text-success-400 border-success-500/30'}`}>
